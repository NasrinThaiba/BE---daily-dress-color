...
app.post("/api/login", async(req,res)=>{
    ...
    if (!customer) {
      res.json({ message: "Email doesn't exist, Authentication Failed" });
  }

  const passwordMatch = await bcrypt.compare(password, customer.password);
  if (!passwordMatch) {
      return res.json({ message: "Password doesn't match, Authentication Failed" });
  }

const token = jwt.sign(
    { email: customer.email, role: "Customers" },
